...
from deletion027_categories_val_a_data import (ARMS, LOSSES, DEFAULT_CONFIG, MILESTONES, validate_config,
    category_prepared, schedule, initial_state, verify_gate, verify_previous_stop)
from deletion027_categories_val_a_worker import aggregate, edit_metrics, evaluate, load_checkpoint, train, case_progress
from deletion027_categories_val_a_analysis import analyze_evaluation, threshold_counts, analyze_available
from deletion027_categories_val_a_report import collect, render, plot_points
from deletion027_worker import weight_hash
from run_027_deletion_categories_val_a import verify_barrier, orchestrate
from test_deletion027_train_ablation import LossTests


def finding(key, half, category="2b", removed_tp=0, removed_fp=0):
    m = edit_metrics(10, 20 if half == "A" else 30, 15, removed_tp, removed_fp)
...
def dashboard_fixture(root):
    ...
    for arm in ARMS:
        ...
        atomic_json(root/"runs"/arm/"status.json", {"status": "evaluating", "update": 100,
            "findings_done": 1, "findings_total": 2, "cases_done": 1, "cases_total": 2})
        metrics = aggregate([finding(arm+"a", "A", arm, 1, 8)])
        atomic_json(root/"runs"/arm/"evaluations/update_0000100/partial.json",
            {"status": "provisional", "update": 100, "findings_done": 1, "findings_total": 2, "cases_done": 1, "cases_total": 2,
    ...
    summary = {"status": "complete", "training_source": "A", "update": 100, "findings_done": 2, "findings_total": 2,
...
class CategoryTests(unittest.TestCase):
    def test_a_only_membership_schedules_patient_separation_and_empty_b(self):
        records, index, halves = [], {}, {}
        for arm in ARMS:
            for j, split in enumerate(("train", "val", "val", "val")):
                key = arm+str(j)
                record = {"key":key,"name":key,"patient":key,"category":arm,"split":split,"voxels":15}
                records.append(record)
                index[key] = {"tiles":[{"start":[0,0,0],"tp":10,"fp":20}],"tp_indices":[0],"fp_indices":[0],
                              "base_tp":10,"base_fp":20,"total_gt":15}
                if split == "val": halves[key] = "A" if j < 3 else "B"
        index['2d3']['tiles'] = []  # Empty prediction remains in B evaluation.
        prepared = category_prepared(records,index,halves)
        for arm in ARMS:
            self.assertEqual(prepared[arm]['train_keys'],[arm+'1',arm+'2'])
            self.assertEqual(len(prepared[arm]['val']),3)
            self.assertEqual(prepared[arm]['exposure'],{'A':'fitted','B':'held_out_development','full':'mixed_exposure'})
            events=schedule(prepared[arm]['train_keys'],index,2000,7)
            self.assertEqual(events,schedule(prepared[arm]['train_keys'],index,2000,7))
            self.assertEqual(events[:100],schedule(prepared[arm]['train_keys'],index,100,7))
            self.assertEqual({e['key'] for e in events},set(prepared[arm]['train_keys']))
            self.assertTrue(all(e['source']=='A' for e in events))
            for start in range(0,2000,100):
                self.assertEqual([sum(r['branch']==b for r in events[start:start+100]) for b in ('tp','fp','eligible')],[50,25,25])
        next(r for r in records if r['key']=='2d3')['patient']='2d1'
        with self.assertRaisesRegex(ValueError,'patient overlap'):category_prepared(records,index,halves)
        index['2b1']['fp_indices']=[]
        self.assertEqual(sum(e['fallback'] for e in schedule(['2b1'],index,100,7)),25)

    def test_case_progress_counts_only_fully_finished_cts(self):
        records=[{'key':'a1','name':'a'},{'key':'a2','name':'a'},{'key':'b1','name':'b'}]
        self.assertEqual(case_progress(records,records[:1]),{'cases_done':0,'cases_total':2})
        self.assertEqual(case_progress(records,records[:2]),{'cases_done':1,'cases_total':2})
        self.assertEqual(case_progress(records,records),{'cases_done':2,'cases_total':2})
        with self.assertRaises(ValueError):case_progress(records,[records[0],records[0]])
    ...
    def test_gate_detects_modified_index_array_or_missing_stop_receipt(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = {'context_sha256': 'c', 'cache_inventory_sha256': 'inventory', 'files': files,
                        'index': index, 'cases': 177, 'findings': 252}
            ...
            with self.assertRaises(ValueError): verify_gate(root, context)
            with self.assertRaises(FileNotFoundError): verify_previous_stop({'stop_receipt': str(root/'absent')})
            atomic_json(root/'stop.json', {'status': 'stopped_by_user', 'update': 6000, 'evaluation_count': 4, 'coordinator_exited': False})
            with self.assertRaises(ValueError): verify_previous_stop({'stop_receipt': str(root/'stop.json')})

    def test_four_barriers_variable_cohorts_and_category_identity(self):
        self.assertEqual(MILESTONES, [100,500,1000,2000])
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            for update in MILESTONES:
                for arm in ARMS:
                    ...
                    atomic_json(root/'runs'/arm/f'evaluations/update_{update:07d}/summary.json',
                        {'status':'complete','training_source':'A','arm':arm,'loss_spec':LOSSES[arm],'update':update,'context_sha256':'c',
                         'checkpoint_sha256':sha256(ck),'findings':prepared[arm]['val']})
                verify_barrier(root, update, prepared, 'c')
            path = root/'runs/2d/evaluations/update_0002000/summary.json'
            value = read_json(path);value['findings'].pop();atomic_json(path,value)
            with self.assertRaises(ValueError):verify_barrier(root,2000,prepared,'c')
    ...
    def test_complete_only_dashboard_independent_publication_and_failures(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(len(plot_points(data['arms'][0], .5, 'A')),1)
            self.assertIsNone(data['arms'][0]['pending_update'])
            self.assertEqual(plot_points(data['arms'][1], .5, 'B'),[])
            ...
            for token in ['SYNTHETIC','pending','0.50','0.90','fitted','failure','held-out development','1/2 CTs','A/B/full']:
    ...
    def test_config_pristine_initial_state_and_gpu_guards(self):
        ...
        with patch('run_027_deletion_categories_val_a.subprocess.Popen') as popen:
        ...
        with tempfile.TemporaryDirectory() as directory, patch('deletion027_categories_val_a_data.check_context', return_value=context):
            root=Path(directory)
    ...
    def test_host_dry_run_never_calls_docker(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result=subprocess.run(['bash',str(REPO/'scripts/rexgroundingct/run_027_deletion_categories_val_a_host.sh'),'orchestrate','--dry-run'],
...
class ReplayTests(unittest.TestCase):
    def test_evaluation_resume_and_dense_analysis_without_repeat_inference(self):
        ...
        with tempfile.TemporaryDirectory() as directory, ExitStack() as stack:
            ...
            torch.save({"model": model.state_dict(), "context_sha256": "test", "update": 100,
                        "training_source": "A", "arm": ARMS[0], "loss_spec": LOSSES[ARMS[0]]}, checkpoint)
            for name, value in (("check_context", context), ("verify_gate", {}), ("setup_device", torch.device("cpu"))):
                stack.enter_context(patch("deletion027_categories_val_a_worker."+name, return_value=value))
            stack.enter_context(patch("deletion027_categories_val_a_worker.make_editor", return_value=model))
            stack.enter_context(patch("deletion027_categories_val_a_worker.FindingStore", return_value=SimpleNamespace(
                get=lambda key: (np.zeros_like(z), z, target, {}, {"ct_metadata": {}}))))
            stack.enter_context(patch("deletion027_categories_val_a_worker.restore_crop_to_original", side_effect=lambda a, _: a))
            stack.enter_context(patch("torch.cuda.max_memory_allocated", return_value=0))
            stack.enter_context(patch("deletion027_categories_val_a_worker.signal.signal"))
            answer = (np.full_like(z, .5), {"tiles": 1, "active_tiles": 1})
            with patch("deletion027_categories_val_a_worker.infer_removal", side_effect=[answer, InterruptedError("synthetic interruption")]):
            ...
            with patch("deletion027_categories_val_a_worker.infer_removal", return_value=answer) as inference:
            ...
            self.assertEqual(result["cases_done"],4)
            self.assertEqual(result["cases_total"],4)
            self.assertEqual(result["training_source"],"A")
            ...
            table=(root/"reports/threshold_sweep"/ARMS[0]/"update_0000100/all_subsets_table.md").read_text()
            self.assertEqual(table.count("| 1.000 |"),3)
            self.assertIn("B is held-out development",table)
    ...
    def test_real_training_loop_interruption_and_resume(self):
        ...
        index = {"a": {"base_tp": 16, "base_fp": 80, "total_gt": 24, "record": {"voxels": 24, "split": "val", "category": ARMS[2]}}}
        events = [{"update": i, "key": "a", "tile_index": 0, "tp": 8, "fp": 56, "source": "A"} for i in range(1, 2201)]
        ...
        with tempfile.TemporaryDirectory() as directory, ExitStack() as stack:
            for name, value in (("check_context", context), ("verify_gate", {"index": index}),
                ...
                stack.enter_context(patch("deletion027_categories_val_a_worker."+name, return_value=value))
            stack.enter_context(patch("deletion027_categories_val_a_worker.FindingStore"))
            stack.enter_context(patch("deletion027_categories_val_a_worker.make_editor", side_effect=lambda *_: torch.nn.Conv3d(2, 1, 1)))
            stack.enter_context(patch("torch.cuda.synchronize"))
            stack.enter_context(patch("torch.cuda.max_memory_allocated", return_value=0))
            handlers = {}
            stack.enter_context(patch("deletion027_categories_val_a_worker.signal.signal", side_effect=lambda s, h: handlers.update({s: h})))
            ...
            with patch("deletion027_categories_val_a_worker.load_event", return_value=(x, y, valid)):
            ...
            def interrupting_load(store, event, size):
                ...
                return x, y, valid
            with patch("deletion027_categories_val_a_worker.load_event", side_effect=interrupting_load):
                with self.assertRaises(InterruptedError):
