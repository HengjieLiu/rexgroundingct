...
if [[ "$command" == "orchestrate" && "${START_GPU_WORK:-0}" != "1" ]]; then
  exec python "$repo_root/scripts/rexgroundingct/run_027_deletion_loss_ablation.py" orchestrate --dry-run "$@"
fi
...
if [[ "${DETACH:-0}" == "1" ]]; then
  detach_args=(--detach --name "${DELETION_ABLATION_CONTAINER_NAME:-rex027_deletion_loss_ablation_a_20ep}")
fi
exec docker run "${detach_args[@]}" "${gpu_args[@]}" --ipc=host --shm-size=16g \
  ...
  "$image_id" python "$repo_root/scripts/rexgroundingct/run_027_deletion_loss_ablation.py" \
  "$command" "${allow_args[@]}" "$@"
