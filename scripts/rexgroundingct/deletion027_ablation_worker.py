"""FP32 loss-ablation workers; shared legacy geometry/metrics with separate state."""
...
from deletion027_ablation_data import (LOSSES, check_context, initial_state, load_event, verify_gate)
        ...
        try:
            for event in fixed["events"][update:until]:
                ...
                row = {**event, "scheduled_tp": event["tp"], "scheduled_fp": event["fp"],
                       "loss_arm": arm, "loss": float(loss.detach()), **{k: float(v.detach()) for k, v in parts.items()},
