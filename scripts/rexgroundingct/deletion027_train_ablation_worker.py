"""FP32 train-only loss workers; historical loss/geometry with separate state."""
...
from deletion027_train_ablation_data import (LOSSES, check_context, initial_state, load_event, verify_gate)
...
def save_checkpoint(folder, model, optimizer, update, history, context, schedule_hash, initial_hash, *, interrupted=False):
    ...
    state = {"model": model.state_dict(), "optimizer": optimizer.state_dict(), "rng": rng_state(),
             ...
             "arm": folder.name, "loss_spec": LOSSES[folder.name]}
    if path.exists():
...
def load_checkpoint(folder, context, schedule_hash):
    ...
    if state.get("arm") != folder.name or state.get("loss_spec") != LOSSES[folder.name]:
...
def train(config, root, arm, until, allow_gpu):
    ...
    with lock(folder / ".train.lock"):
        ...
        if (fixed["sha256"] != digest(fixed["events"]) or fixed["context_sha256"] != context["sha256"]
                or fixed["sha256"] != config["schedule_sha256"]
        ...
        try:
            for event in fixed["events"][update:until]:
                if event["source"] != "train" or manifest["index"][event["key"]]["record"]["split"] != "train":
                ...
                counts = {"base_tp": item["base_tp"], "base_fp": item["base_fp"], "total_gt": item["record"]["voxels"]}
                loss, parts = loss_for_arm(logits, x[:, 1:2], y, valid, counts, arm)
                ...
                row = {**event, "scheduled_tp": event["tp"], "scheduled_fp": event["fp"],
                       "loss_arm": arm, "training_source": "train", "loss": float(loss.detach()),
    ...
    with lock(out / ".eval.lock"):
        ...
        if (out / "summary.json").exists():
            ...
            if (summary["checkpoint_sha256"] != checksum or summary["context_sha256"] != context["sha256"]
                    or summary["arm"] != arm or len(summary["findings"]) != 69
        ...
        def publish(complete=False):
            summary = {"status": "complete" if complete else "provisional", "arm": arm, "update": update,
                       ...
                       "exposure": {h: "held_out_development" for h in ("A", "B", "full")},
