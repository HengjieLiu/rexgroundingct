...
from deletion027_categories_data import ARMS, LOSSES, DEFAULT_CONFIG, check_context, create_context, verify_gate, initial_state, verify_2a_stop
...
def orchestrate(config, args):
    if args.dry_run:
        print({"gpu_work_started": False, "arms": ARMS, "losses": LOSSES,
               ...
               "cache_gate": "2730 CTs / 5106 findings; completed cache reuse, no inference", "updates_per_arm": config["total_updates"],
    ...
    with lock(root / ".orchestrator.lock"):
        ...
        try:
            ...
            from deletion027_categories_analysis import audit
...
def main():
    ...
    if args.command == "orchestrate":
        ...
        from deletion027_categories_worker import train, evaluate
        (train if args.command == "train" else evaluate)(config, root, args.arm, args.update, args.allow_gpu)
    elif args.command == "analyze":
        from deletion027_categories_analysis import analyze_available, watch
        if args.watch:
            watch(config, root)
        else:
        ...
        from deletion027_categories_report import render, watch
        if args.watch:
