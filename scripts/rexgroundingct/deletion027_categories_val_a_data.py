...
DEFAULT_CONFIG = REPO / "configs/experiments/027_deletion_categories_bcde_val_a_20ep.json"
MILESTONES = [100, 500, 1000, 2000]


def code_manifest():
    names = ("deletion027_categories_val_a_data.py", "deletion027_categories_val_a_worker.py",
             "deletion027_categories_val_a_report.py", "deletion027_categories_val_a_analysis.py",
             "run_027_deletion_categories_val_a.py", "run_027_deletion_categories_val_a_host.sh",
...
def validate_config(config, base):
    wanted = {"arms": list(ARMS), "loss_arm": "dice_bce_tp2", "precision": "fp32", "tf32": False,
              "total_updates": 2000, "updates_per_epoch": 100, "evaluation_updates": MILESTONES,
              "auxiliary_bce_weight": .25, "dice_smoothing": 1e-6,
              "initial_remove_probability": .05, "training_source": "A",
              "display_thresholds": [.5, .9], "thresholds": [.5, .8, .9, .95, .99, 1.],
              "validation_plot_policy": "complete_category_only", "validation_plot_subsets": ["A", "B"]}
...
def category_prepared(records, index, halves):
    from deletion027_worker import edit_metrics
    result = {}
    for arm in ARMS:
        val = [{**r, "half": halves[r["name"]]} for r in records if r["category"] == arm and r["split"] == "val"]
        train = [r for r in val if r["half"] == "A"]
        other = [r for r in val if r["half"] == "B"]
        if not train or not other or len(train)+len(other) != len(val):
            raise ValueError("Incomplete A/B category membership")
        if {r["patient"] for r in train} & {r["patient"] for r in other}:
            raise ValueError("A/B patient overlap")
        if len({r["key"] for r in val}) != len(val):
            raise ValueError("Duplicate finding identifiers")
        keys = [r["key"] for r in train if index[r["key"]]["tiles"]]
        if len(keys) != len(train):
            raise ValueError("The verified A cohort should contain only eligible findings")
        baseline = []
        for r in val:
            item = index[r["key"]]
            if item["total_gt"] != r["voxels"]:
                raise ValueError("Full-finding GT count changed")
        ...
        result[arm] = {"train": train, "val": val, "train_keys": keys, "baseline_findings": baseline,
                       "exposure": {"A": "fitted", "B": "held_out_development", "full": "mixed_exposure"}}
    return result
...
def schedule(keys, index, updates, seed):
    ...
    for _ in range(updates // 100):
        ...
        for branch in branches:
            ...
            events.append({"update": len(events)+1, "source": "A", "key": key, "branch": branch,
...
def create_context(config, root):
    ...
    proofs = [p for p in inventory["case_proofs"] if p["name"] in names]
    if len(proofs) != 177 or len(records) != 252:
        raise ValueError("Complete validation-only cache required")
    index, files = {}, {}
    for i, proof in enumerate(proofs):
        ...
        if i % 50 == 0 or i+1 == len(proofs):
            atomic_json(root / "preparation_progress.json", {"verified_cases": i+1, "total_cases": 177, "at": now()})
    if set(index) != {r["key"] for r in records}:
        raise ValueError("Index coverage differs")
    prepared = category_prepared(records, index, source["halves"])
    schedules = {}
    for arm in ARMS:
        if (len(prepared[arm]["train_keys"]) != config["expected_eligible_train"][arm]
                or len(prepared[arm]["val"]) != config["expected_val"][arm]
                or len({r["name"] for r in prepared[arm]["val"]}) != config["expected_val_cts"][arm]):
        ...
        if {e["key"] for e in schedules[arm]} != set(prepared[arm]["train_keys"]):
            raise ValueError("Every A finding must appear in the fixed schedule")
    ...
    manifest = {"context_sha256": context["sha256"], "cache_inventory_sha256": config["cache_inventory_sha256"],
                ...
                "eligible_counts": config["expected_eligible_train"], "cases": 177, "findings": 252}
...
def verify_gate(root, context):
    ...
    if (manifest["context_sha256"] != context["sha256"]
            ...
            or (manifest["cases"], manifest["findings"]) != (177, 252)
...
def verify_previous_stop(config):
    receipt = read_json(config["stop_receipt"])
    if (receipt.get("status") != "stopped_by_user" or receipt.get("update") != 2000
            or receipt.get("updates") != {a: 2000 for a in ARMS}
            or not receipt.get("coordinator_exited") or receipt.get("container_state", {}).get("Running", True)
            or receipt.get("evaluation_2000_started") is not False
            or len(receipt.get("checkpoints", {})) != 4):
        raise ValueError("Verified previous category stop at update 2000 without validation required")
    for path, checksum in receipt["checkpoints"].items():
