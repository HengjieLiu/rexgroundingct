...
CONTAINER_NAME="${CONTAINER_NAME:-rex009_s3_attention_$TIMESTAMP}"
DETACH="${DETACH:-1}"
...
for key in EXP_DIR EXP006_DIR EXP007_DIR CACHE_ROOT SOURCE_RUN_GROUP \
  ...
  VAL20_JSON VAL200_JSON CHECKPOINT_UPDATES SEGMENT_EPOCHS RUN_STATIC_ONLY \
  RUN_SMOKE_ONLY RUN_FULL RUN_MODEL_MATERIALIZATION RUN_TRAIN_SMOKES \
  RUN_EPOCH0_EVAL EPOCH0_DICE_TOLERANCE ARM_STAGGER_SECONDS \
  S3_COUPLING_RAMP_UPDATES; do
  if [[ -n "${!key:-}" ]]; then
...
"${docker_args[@]}" "$IMAGE" \
  bash /workspace/scripts/rexgroundingct/run_009_s3_attention_coupling_ablation.sh
