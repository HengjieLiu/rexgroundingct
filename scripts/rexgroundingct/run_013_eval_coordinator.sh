...
arm_dataset() {
  case "$EVAL_ARM:$EVAL_SCOPE" in
    category_1all_diffuse_target100:target)
      echo /workspace/configs/evaluation/rexgroundingct_val42_category_1all_diffuse_target_census.json ;;
    category_2all_focal_target100:target)
      echo /workspace/configs/evaluation/rexgroundingct_val196_category_2all_focal_target_census.json ;;
    category_2bc_target100:target)
      echo /workspace/configs/evaluation/rexgroundingct_val76_category_2bc_target_census.json ;;
    category_2d_target100:target)
...
(
  ...
  EVAL_LOCK_HELD=1 GLOBAL_ONLY=1 EVAL_DATASET_JSON_OVERRIDE="$dataset" \
    EVAL_LABEL="Experiment 013 $EVAL_ARM epoch $EVAL_EPOCH $EVAL_SCOPE threshold 0.5" \
