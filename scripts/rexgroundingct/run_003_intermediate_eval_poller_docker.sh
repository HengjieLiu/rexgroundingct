...
CONTAINER_NAME="${CONTAINER_NAME:-rex003_intermediate_eval_poller_${TIMESTAMP}}"
DETACH="${DETACH:-1}"
POLLER_LOG="${POLLER_LOG:-$EXP_DIR/logs/intermediate_eval_poller_${RUN_GROUP}_${TIMESTAMP}.log}"
...
forward_env=(
  ...
  STEPS_PER_EPOCH
  POLL_SECONDS
  CHECKPOINT_STABILITY_SECONDS
  EVAL_LOCK_POLL_SECONDS
  EVAL_LOCK_STALE_SECONDS
  DRY_RUN
  EMBEDDINGS
  VAL20_JSON
  VAL200_JSON
...
"${docker_args[@]}" "$IMAGE" \
  bash -lc 'bash /workspace/scripts/rexgroundingct/run_003_intermediate_eval_poller.sh 2>&1 | tee "$POLLER_LOG"'
