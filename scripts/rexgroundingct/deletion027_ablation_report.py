"""Single CPU writer for the A-only loss comparison, with provisional A/B scores."""
...
from deletion027_ablation_data import ARMS, LOSSES
from deletion027_ablation_worker import aggregate
...
def render(root):
    ...
    title = ("SYNTHETIC FIXTURE — NOT EXPERIMENT RESULTS" if data["state"].get("synthetic_fixture")
             else "Exp027 — four losses fitted on A")
    fig.suptitle(f"{title}\n{data['state']['status']} | A: fitting; B: held-out development", fontsize=14)
    ...
    lines = [f"# {title}", "", f"Refreshed: {data['updated_at']}", "",
             f"Status: **{data['state']['status']}**. Stop at update 2,000 after all final evaluations and analysis.", "",
             "![Live 3×3 curves](live_dashboard.png)", "",
             "All four arms fit the same 35 A findings; 34 B findings are excluded from gradients. "
             "B has already informed development and is not an untouched final test. Full results mix exposure.", "",
             ...
             "| Arm | Objective | Phase | Update / epoch | Total / F / K / D | Step s | Elapsed s | Peak GiB | Progress |",
             "| --- | --- | --- | --- | --- | --- | --- | --- | --- |"]
    for item in data["arms"]:
        ...
        lines.append(f"| {item['arm']} | {LOSSES[item['arm']]['formula']} | {state['status']} | "
                     f"{update} / {update/100:.2f} | {losses} | {fmt(row.get('step_seconds'))} | "
                     f"{fmt(state.get('elapsed_seconds'))} | {fmt(memory/2**30 if memory is not None else None)} | {progress} |")
    if data["preparation"]:
        lines += ["", "CPU validation-cache verification: "+"; ".join(
        ...
        for item in data["arms"]:
            ...
            if result:
                ...
                gaps.append(f"{item['arm']} A-minus-B improvement gap: {fmt(gap)} "
                            f"({'provisional' if item['partial'] else 'complete'}; descriptive, not proof of overfitting).")
        for gap in gaps:
            lines += ["", gap]
