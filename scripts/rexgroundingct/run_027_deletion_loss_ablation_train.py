#!/usr/bin/env python3
"""Four train-only loss arms with GPU opt-in, verified barriers and CPU reports."""
...
from deletion027_train_ablation_data import ARMS, LOSSES, DEFAULT_CONFIG, check_context, create_context, finish_preparation, prepare_shard
...
def orchestrate(config, args):
    if args.dry_run:
        print({"gpu_work_started": False, "arms": ARMS, "losses": LOSSES,
               "training_pool": "1119 eligible original train findings; no A/B gradients",
               "cache_gate": "864 CTs / 1189 findings; no inference", "updates_per_arm": config["total_updates"],
        ...
        try:
            ...
            for milestone in config["evaluation_updates"]:
                if milestone in prior:
                    ...
                    from deletion027_train_ablation_analysis import verify_reference
                    verify_reference(config, root, milestone)
                    continue
            ...
            from deletion027_train_ablation_analysis import audit
...
def main():
    ...
    if args.command == "orchestrate":
        ...
        from deletion027_train_ablation_worker import train, evaluate
        (train if args.command == "train" else evaluate)(config, root, args.arm, args.update, args.allow_gpu)
    elif args.command == "analyze":
        from deletion027_train_ablation_analysis import analyze_available, watch
        if args.watch:
            watch(config, root)
        else:
        ...
        from deletion027_train_ablation_report import render, watch
        if args.watch:
