#!/usr/bin/env python3
"""Build the atomic live report and target-only selection for Exp012."""
...
from prepare_012_category_specialists import ARM_TARGETS


EXPERIMENT_ID = "012_voxtell_category_specialists_replay50_cont100"
MILESTONES = (0, 5, 20, 40, 60, 80, 100)
EVAL_SCOPE = {0: "union", 5: "target", 20: "union", 40: "union", 60: "union", 80: "union", 100: "val200"}
EXPECTED_BASELINES = {
    "category_1alldiffuse_replay50": {"dice": 0.16273172502327543, "hits": 19, "findings": 52},
    "category_2a_replay50": {"dice": 0.3405199216031458, "hits": 57, "findings": 69},
...
def render_markdown(summary: dict[str, Any]) -> str:
    ...
    for arm, arm_data in summary["arms"].items():
        cells = [target_cell(arm_data["milestones"][str(epoch)]) for epoch in MILESTONES]
    ...
    for arm, arm_data in summary["arms"].items():
        lines.extend([f"### `{arm}`", "", "| Category | Findings | Dice | Hit rate |", "| --- | ---: | ---: | ---: |"])
        evaluation = arm_data["milestones"]["100"].get("evaluation")
