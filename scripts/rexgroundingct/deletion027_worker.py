"""Independent FP32 trainers and idempotent full-volume deletion evaluators."""
...
from fit_027_deletion import deletion_loss, infer_removal, make_editor
from deletion027_data import check_context, load_event


def edit_metrics(base_tp, base_fp, total_gt, removed_tp, removed_fp):
    tp, fp = base_tp - removed_tp, base_fp - removed_fp
    if min(tp, fp, removed_tp, removed_fp, total_gt - base_tp) < 0:
        raise ValueError("Invalid deletion counts")
    eps = 1e-6
    before = (2*base_tp + eps)/(base_tp + base_fp + total_gt + eps)
    after = (2*tp + eps)/(tp + fp + total_gt + eps)
    return {"base_tp": int(base_tp), "base_fp": int(base_fp), "total_gt": int(total_gt),
            "tp": int(tp), "fp": int(fp), "fn": int(total_gt - tp),
            "tp_removed": int(removed_tp), "fp_removed": int(removed_fp),
            "base_dice": before, "dice": after, "delta_dice": after - before,
            "base_hit": int(before >= .1), "hit": int(after >= .1),
            "precision": tp/(tp+fp) if tp+fp else 0., "recall": tp/total_gt if total_gt else None,
            "tp_retention": tp/base_tp if base_tp else None, "fp_removal": removed_fp/base_fp if base_fp else None,
            "deletion_precision": removed_fp/(removed_tp+removed_fp) if removed_tp+removed_fp else None,
            "new_foreground": 0}
...
def aggregate(rows):
    ...
    for half in ("A", "B", "full"):
        ...
        if not subset:
            result[half] = {"findings": 0, **{k: None for k in ("dice", "base_dice", "hit_rate", "base_hit_rate", "tp_retention", "fp_removal")}}
            continue
        n = len(subset)
        out = {"findings": n, "cases": len({r["name"] for r in subset})}
        for key in ("base_tp", "base_fp", "tp", "fp", "fn", "tp_removed", "fp_removed", "hit", "base_hit"):
            out[key] = sum(r[key] for r in subset)
        for key in ("dice", "base_dice", "delta_dice", "precision", "recall"):
            values = [r[key] for r in subset if r[key] is not None]
            out[key] = math.fsum(values)/len(values) if values else None
        retained = [r["tp_retention"] for r in subset if r["tp_retention"] is not None]
        out.update(hit_rate=out["hit"]/n, base_hit_rate=out["base_hit"]/n,
                   tp_retention=out["tp"]/out["base_tp"] if out["base_tp"] else None,
                   fp_removal=out["fp_removed"]/out["base_fp"] if out["base_fp"] else None,
                   deletion_precision=out["fp_removed"]/(out["fp_removed"]+out["tp_removed"]) if out["fp_removed"]+out["tp_removed"] else None,
                   tp_retention_mean=float(np.mean(retained)) if retained else None,
                   tp_retention_median=float(np.median(retained)) if retained else None,
                   tp_retention_p10=float(np.quantile(retained, .1)) if retained else None,
                   tp_retention_min=min(retained) if retained else None,
                   below_95=sum(v < .95 for v in retained), below_80=sum(v < .8 for v in retained),
                   undefined_retention=n-len(retained),
                   improved=sum(r["delta_dice"] > 1e-12 for r in subset),
                   worsened=sum(r["delta_dice"] < -1e-12 for r in subset),
                   unchanged=sum(abs(r["delta_dice"]) <= 1e-12 for r in subset))
        result[half] = out
    return result
...
def initial_state(config, root):
    ...
    if path.exists():
        ...
        if value["context_sha256"] != context["sha256"]:
            raise ValueError("Initial state context differs")
        return value
    torch.manual_seed(config["seed"])
    model = make_editor(config, context["base_config"])
    value = {"model": model.state_dict(), "weights_sha256": weight_hash(model), "context_sha256": context["sha256"]}
    save_torch(path, value)
    return value


def setup_device(allow_gpu):
    if not allow_gpu or os.environ.get("START_GPU_WORK") != "1":
        raise ValueError("GPU work requires explicit opt-in")
    if torch.cuda.device_count() != 1:
        raise ValueError("Worker must see exactly one assigned GPU")
...
def status(folder, phase, **extra):
    atomic_json(folder / "status.json", {"status": phase, "updated_at": now(), **extra})
...
def train(config, root, arm, until, allow_gpu):
    ...
    if manifest["context_sha256"] != context["sha256"] or manifest["findings"] != 1189:
        raise ValueError("Complete input gate missing")
    ...
    with lock(folder / ".train.lock"):
        ...
        if (folder / "latest_checkpoint.json").exists():
            ...
            seed = config["seed"] + 701 * (list(__import__('exp027_common').ARMS).index(arm)+1)
        ...
        store = FindingStore(Path(base["experiment_dir"]), prepared, base, capacity=2)
        ...
        try:
            for event in fixed["events"][update:until]:
                ...
                torch.cuda.synchronize()
                update = event["update"]
                row = {**event, "loss": float(loss.detach()), **{k: float(v.detach()) for k, v in parts.items()},
...
def evaluate(config, root, arm, update, allow_gpu):
    ...
    with lock(out / ".eval.lock"):
        ...
        if (out / "summary.json").exists():
            ...
            if summary["checkpoint_sha256"] != checksum or {r["key"] for r in summary["findings"]} != {r["key"] for r in prepared["val"]}:
        ...
        def publish(complete=False):
            summary = {"status": "complete" if complete else "provisional", "arm": arm, "update": update,
                       "checkpoint_sha256": checksum, "context_sha256": context["sha256"],
        ...
        try:
            ...
            for record in prepared["val"]:
                ...
                result = {**record, "thresholds": thresholds,
                          ...
                          "checkpoint_sha256": checksum, "scores_sha256": sha256(score_path),
