...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/deletion027_categories_val_a_matplotlib")
import matplotlib
matplotlib.use("Agg")
...
from deletion027_categories_val_a_data import ARMS
from deletion027_categories_val_a_worker import aggregate
...
def collect(root):
    ...
    for arm in ARMS:
        ...
        pending = state.get("update") if state.get("status") in ("evaluating", "waiting_at_evaluation_barrier") and state.get("update") not in final_updates else None
        items.append({"arm": arm, "history": read_updates(folder / "training.jsonl"), "state": state,
                      ...
                      "expected_cases": len({r["name"] for r in prepared.get(arm, {}).get("val", [])}),
...
def plot_points(item, threshold, subset):
    if subset not in ("A", "B"):
        raise ValueError("Validation curves must separate A and B")
    return [(e["update"], e["threshold_metrics"][str(threshold)][subset]["dice"]) for e in item["evaluations"]]
...
def render(root):
    ...
    for item, color, position in zip(data["arms"], COLORS, ((1, 1), (1, 2), (2, 1), (2, 2))):
        ax = axes[position]
        ax.set_title(f"{item['arm']}: A fitted / B development Dice")
        for subset, baseline_style, baseline_color in (("A", "-.", "#777777"), ("B", ":", "black")):
            baseline = item["baseline"][subset]["dice"]
            if baseline is not None:
                ax.axhline(baseline, color=baseline_color, linestyle=baseline_style,
                           label=f"{subset} base {baseline:.6f}")
            for threshold, marker in ((.5, "o"), (.9, "s")):
                points = plot_points(item, threshold, subset)
                if points:
                    ax.plot(*zip(*points), color=color, linestyle="--" if subset == "A" else "-",
                            marker=marker, markerfacecolor="none" if subset == "A" else color,
                            label=f"{subset} removal {threshold:.2f}")
        if item["pending_update"] is not None:
            state = item["state"]
            ax.text(.03, .97, f"Update {item['pending_update']}: pending\n{state.get('cases_done', 0)}/{item['expected_cases']} CTs; {state.get('findings_done', 0)}/{item['expected_findings']} findings",
    ...
    title = "SYNTHETIC FIXTURE — NOT EXPERIMENT RESULTS" if data["state"].get("synthetic_fixture") else "Exp027 — A-only category Dice + BCE TP2"
    fig.suptitle(f"{title}\n{data['state']['status']} | A/B points only after complete category evaluation", fontsize=14)
    ...
    lines = [f"# {title}", "", f"Refreshed: {data['updated_at']}", "", f"Status: **{data['state']['status']}**.", "",
             ...
             "Each category fits validation A only for 2000 updates (20 epochs). "
             ...
             "the preceding completed points remain visible. A is fitted (dashed/hollow); B is held-out development (solid/filled). "
             "Circles mark 0.50 and squares 0.90. A/B/full metrics use the same finding records. Full results mix exposure. "
             "Thresholds do not enter training. Reload the viewer if needed.", "",
             "All original-training findings are unused; all validation findings remain. "
             "A/B retain the patient partition. B has informed prior development; category 2e B has only three findings.", "",
    ...
    for item in data["arms"]:
        ...
        progress = (f"{state.get('cases_done', 0)}/{item['expected_cases']} CTs; "
                    f"{state.get('findings_done', 0)}/{item['expected_findings']} findings" if "findings_total" in state else "pending")
    ...
    lines += ["", "## Latest completed A/B/full results", "",
              "No numeric result from an unfinished category evaluation is displayed.", "",
              "| Category | Subset / exposure | Completed update | Baseline Dice | Dice at 0.50 | Change | Dice at 0.90 | Change | Next evaluation |",
              "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |"]
    for item in data["arms"]:
        ...
        if final and final["update"] == 2000:
            pending = "finished"
        for subset, exposure in (("A", "fitted"), ("B", "held-out development"), ("full", "mixed exposure")):
            metrics = [final["threshold_metrics"][str(t)][subset] if final else {} for t in (.5, .9)]
            lines.append(f"| {item['arm']} | {subset}: {exposure} | {final['update'] if final else 'pending'} | "
                         f"{fmt(item['baseline'][subset]['dice'])} | {fmt(metrics[0].get('dice'))} | "
                         f"{fmt(metrics[0].get('delta_dice'))} | {fmt(metrics[1].get('dice'))} | "
                         f"{fmt(metrics[1].get('delta_dice'))} | {pending} |")
