#!/usr/bin/env python3
"""Four category-specific Dice + BCE TP2 models with GPU opt-in, verified barriers and CPU reports."""
...
from deletion027_categories_val_a_data import ARMS, LOSSES, DEFAULT_CONFIG, check_context, create_context, verify_gate, initial_state, verify_previous_stop
...
def verify_barrier(root, update, prepared, context_hash):
    for arm in ARMS:
        expected = {r["key"] for r in prepared[arm]["val"]}
        ...
        if (summary.get("status") != "complete" or summary["context_sha256"] != context_hash or summary["update"] != update
                or summary.get("training_source") != "A" or summary.get("arm") != arm or summary.get("loss_spec") != LOSSES[arm]
...
def orchestrate(config, args):
    if args.dry_run:
        print({"gpu_work_started": False, "arms": ARMS, "losses": LOSSES,
               "training_pool": config["expected_eligible_train"],
               "cache_gate": "177 CTs / 252 findings; completed cache reuse, no inference", "updates_per_arm": config["total_updates"],
    ...
    with lock(root / ".orchestrator.lock"):
        ...
        try:
            ...
            atomic_json(root / "preparation.json", {"seconds": time.perf_counter()-started,
                        "cases": manifest["cases"], "findings": manifest["findings"],
                        "eligible_counts": manifest["eligible_counts"], "initial_sha256": initial["weights_sha256"]})
            ...
            from deletion027_categories_val_a_analysis import audit
            completion = audit(config, root)
            group_status(root, "pending_user_review", update=config["total_updates"],
                         completed_evaluations=completion["evaluations"], elapsed_seconds=time.perf_counter()-started)
...
def main():
    ...
    parser.add_argument("command", choices=("orchestrate", "prepare", "train", "evaluate", "report", "analyze", "audit"))
    ...
    if args.command == "orchestrate":
        ...
        from deletion027_categories_val_a_worker import train, evaluate
        (train if args.command == "train" else evaluate)(config, root, args.arm, args.update, args.allow_gpu)
    elif args.command == "analyze":
        from deletion027_categories_val_a_analysis import analyze_available, watch
        if args.watch:
            watch(config, root)
        else:
        ...
        from deletion027_categories_val_a_report import render, watch
        if args.watch:
