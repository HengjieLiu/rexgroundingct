...
CONTAINER_NAME="${CONTAINER_NAME:-rex004_native_vs_2mm_$TIMESTAMP}"
DETACH="${DETACH:-1}"
...
for key in EXP_DIR EXP003_DIR SOURCE_GROUP SOURCE_CHECKPOINT SOURCE_PLANS SOURCE_SCHEDULE \
  SEED RUN_GROUP GROUP_DIR CACHE_ROOT CACHE_WORKERS SCHEDULE_WORKERS EMBEDDINGS \
  VAL20_JSON VAL200_JSON POLL_SECONDS STABILITY_SECONDS; do
  if [[ -n "${!key:-}" ]]; then
...
"${docker_args[@]}" "$IMAGE" \
  bash /workspace/scripts/rexgroundingct/run_004_native_vs_2mm_continuation.sh
