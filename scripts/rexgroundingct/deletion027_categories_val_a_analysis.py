...
from deletion027_categories_val_a_data import ARMS, LOSSES, check_context
from deletion027_categories_val_a_worker import aggregate, edit_metrics
...
def analyze_evaluation(config, root, arm, summary_path):
    ...
    if marker.exists():
        ...
        return done
    if summary["status"] != "complete" or summary.get("training_source") != "A" or summary["arm"] != arm or len(summary["findings"]) != config["expected_val"][arm]:
    ...
    fig.suptitle(f"{arm}, update {summary['update']} — fitted on A; B held-out development\nRetrospective curves; no operating point selected")
...
def analyze_available(config, root):
    ...
    lines = ["# Category threshold sweeps", "", "Separate A/B figures; A fitted, B held-out development, full mixed exposure. A/B/full metrics retained in CSV/JSON. "
             "All thresholds are retrospective diagnostics, with no automatic selection.", "",
             "| Category | Update | All thresholds with baseline | Per-finding records | A/B tradeoffs |",
             "| --- | --- | --- | --- | --- |"]
    for result in found:
        ...
        table = [f"# {result['arm']} update {result['update']}: A/B/full threshold sweep", "",
                 "A is fitted, B is held-out development, full mixes exposure; no extra inference was used.", "",
                 "| Subset | Removal threshold | Dice | Baseline Dice | Change | TP loss | FP removal |",
                 "| --- | ---: | ---: | ---: | ---: | ---: | ---: |"]
        for row in metrics:
            if row["subset"] in ("A", "B", "full"):
                tp = "pending" if row["tp_loss"] is None else f"{row['tp_loss']:.6f}"
                fp = "pending" if row["fp_removal"] is None else f"{row['fp_removal']:.6f}"
                table.append(f"| {row['subset']} | {row['threshold']:.3f} | {row['dice']:.6f} | {row['base_dice']:.6f} | {row['delta_dice']:+.6f} | {tp} | {fp} |")
        atomic_text(folder / "all_subsets_table.md", "\n".join(table) + "\n")
        lines.append(f"| {result['arm']} | {result['update']} | [Table]({relative}/all_subsets_table.md) / [CSV]({relative}/metrics.csv) | "
...
def audit(config, root):
    ...
    from run_027_deletion_categories_val_a import verify_barrier
    ...
    for arm in ARMS:
        ...
        for event, row in zip(fixed["events"], history):
            if (any(row[k] != v for k, v in event.items() if k not in ("tp", "fp"))
                    ...
                    or row["training_source"] != "A" or row["key"] not in prepared[arm]["train_keys"]):
        ...
        for update in range(0, config["total_updates"] + 1, 100):
            ...
            if (state["update"] != update or state["sampling_cursor"] != update
                    ...
                    or state["training_source"] != "A" or state["precision"] != "fp32" or state["tf32"]):
