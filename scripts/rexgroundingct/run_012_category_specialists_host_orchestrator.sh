...
if [[ "$START_TRAINING" != "1" && "$DRY_RUN" != "1" ]]; then
  echo "START_TRAINING=1 is required to launch Exp012" >&2
  exit 2
fi
...
RUN_GROUP="${RUN_GROUP:-exp012_category_specialists_$TIMESTAMP}"
...
if [[ "$DETACH" == "1" && "${ORCHESTRATOR_CHILD:-0}" != "1" ]]; then
  ...
  echo "Started Exp012 host orchestrator"
  echo "PID: $pid"
  echo "tmux session: $session"
  echo "Run group: $RUN_GROUP"
  echo "Live report: $GROUP_DIR/reports/exp012_progress.md"
  ...
  if ((status != 0)); then
    ...
    if [[ -f "$GROUP_DIR/run_group_manifest.json" ]]; then
      PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$REPO_ROOT/scripts/rexgroundingct" python \
        "$REPO_ROOT/scripts/rexgroundingct/summarize_012_results.py" \
        --group-dir "$GROUP_DIR" \
        --source-checkpoint "$SOURCE_CHECKPOINT" \
        --subset-manifest "$REPO_ROOT/configs/evaluation/rexgroundingct_exp012_category_evaluation_subsets.manifest.json" \
...
run_stage() {
  ...
  local container="rex012_${suffix}_${RUN_GROUP##*_}"
  ...
  "${docker_args[@]}" "$IMAGE" \
    bash /workspace/scripts/rexgroundingct/run_012_category_specialists_stage.sh \
    2>&1 | tee "$stage_log"
...
[[ -f "$GROUP_DIR/.selection_ready" ]] || {
  echo "Exp012 did not reach selection_ready" >&2
  exit 1
}
touch "$GROUP_DIR/.orchestrator_complete"
echo "Exp012 paused at selection_ready: $GROUP_DIR"
