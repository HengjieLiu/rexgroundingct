...
from deletion027_categories_val_a_data import (LOSSES, check_context, initial_state, load_event, verify_gate)
...
def save_checkpoint(folder, model, optimizer, update, history, context, schedule_hash, initial_hash, *, interrupted=False):
    ...
    state = {"model": model.state_dict(), "optimizer": optimizer.state_dict(), "rng": rng_state(),
             ...
             "training_source": "A", "arm": folder.name, "category": folder.name, "loss_arm": "dice_bce_tp2", "loss_spec": LOSSES[folder.name]}
    if path.exists():
...
def load_checkpoint(folder, context, schedule_hash):
    ...
    if (state.get("training_source") != "A" or state.get("arm") != folder.name or state.get("category") != folder.name
    ...
    with lock(folder / ".train.lock"):
        ...
        try:
            for event in fixed["events"][update:until]:
                if (event["source"] != "A" or event["key"] not in prepared["train_keys"]
                        or manifest["index"][event["key"]]["record"]["category"] != arm
                        or manifest["index"][event["key"]]["record"]["split"] != "val"):
                    raise ValueError("Non-A or wrong-category finding in A-only worker")
                if stopping[0]:
                ...
                row = {**event, "scheduled_tp": event["tp"], "scheduled_fp": event["fp"],
                       "loss_arm": config["loss_arm"], "category": arm, "training_source": "A", "loss": float(loss.detach()),
...
def case_progress(records, completed):
    groups = {}
    for record in records:
        groups.setdefault(record["name"], set()).add(record["key"])
    done = {r["key"] for r in completed}
    if not done <= {r["key"] for r in records} or len(done) != len(completed):
        raise ValueError("Unexpected/duplicate completed finding")
    return {"cases_done": sum(keys <= done for keys in groups.values()), "cases_total": len(groups)}
...
def evaluate(config, root, arm, update, allow_gpu):
    ...
    with lock(out / ".eval.lock"):
        ...
        if (state["context_sha256"] != context["sha256"] or state["update"] != update
                or state.get("training_source") != "A" or state.get("arm") != arm or state.get("loss_spec") != LOSSES[arm]):
        ...
        def publish(complete=False):
            summary = {"status": "complete" if complete else "provisional", "arm": arm, "update": update,
                       ...
                       **case_progress(prepared["val"], rows),
        ...
        try:
            status(folder, "evaluating", update=update, findings_done=0, findings_total=len(prepared["val"]), **case_progress(prepared["val"], rows))
            publish()
            for record in prepared["val"]:
                ...
                if path.exists():
                    ...
                    rows.append(result); publish();
                    status(folder, "evaluating", update=update, findings_done=len(rows), findings_total=len(prepared["val"]), **case_progress(prepared["val"], rows))
                    continue
                ...
                rows.append(result)
                publish()
                status(folder, "evaluating", update=update, findings_done=len(rows), findings_total=len(prepared["val"]), **case_progress(prepared["val"], rows), elapsed_seconds=time.perf_counter()-started, peak_memory_bytes=torch.cuda.max_memory_allocated())
