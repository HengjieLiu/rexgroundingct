...
from deletion027_train_ablation_data import ARMS, LOSSES, check_context
from deletion027_train_ablation_worker import aggregate, edit_metrics
from deletion027_train_ablation_comparison import compare_with_a, threshold_comparisons, verify_reference
...
def analyze_evaluation(config, root, arm, summary_path):
    ...
    fig.suptitle(f"{arm}, update {summary['update']} — trained on original train; A/B held-out development\nRetrospective curves; no operating point selected")
...
def analyze_available(config, root):
    ...
    lines = ["# Saved threshold analysis", "", "Models fit original train only; both A and B are held-out development. "
             "All thresholds are retrospective diagnostics; no model or threshold is selected.", "",
             "## Full comparison tables with baseline", ""]
    for update, count, folder in threshold_comparisons(config, root, found):
        lines.append(f"- [Update {update}: all 201 thresholds, A/B/full]({folder}/all_thresholds.md) "
                     f"({count}/4 arms available).")
    if found:
        compare_with_a(config, root)
    else:
        lines.append("Evaluations pending.")
    lines += ["", "## Per-arm records", "",
...
def audit(config, root):
    ...
    for arm in ARMS:
        ...
        if len(history) != config["total_updates"] or digest(schedule["events"]) != config["schedule_sha256"]:
        ...
        for update in range(0, config["total_updates"]+1, config["updates_per_epoch"]):
            ...
            if arm == ARMS[0] and update in (100, 500, 1000, 2000):
                reference.append(verify_reference(config, root, update))
            del model, state
        for update in config["evaluation_updates"]:
    ...
    if len(analyses) != expected or len(reference) != 4:
    ...
    completion = {"status": "pending_user_review" if identical else "reference_discrepancy_requires_investigation",
                  "updates": len(ARMS)*config["total_updates"], "evaluations": expected, "dense_analyses": expected,
    ...
    if not identical:
        raise ValueError("BCE reference differs from historical train-only run; inspect completion.json before interpreting")
    return completion
